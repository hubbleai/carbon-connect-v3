...
>(({ className, children, activeState, ...props }, ref) => {
  const { zIndex, width, height } = useCarbon();
  const finalWidth = width !== undefined ? width : 784;
  const finalHeight = height !== undefined ? height : 703;
  // All the conditions for width of activeState are done like below

  const dynamicWidthStyle =
    activeState === "CONNECT"
      ? { width: "415px" }
      : { width: "100%", maxWidth: `${finalWidth}px` };
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        {...props}
        style={{ zIndex: zIndex, ...dynamicWidthStyle, height: finalHeight }}
      >
        {children}
      </DialogPrimitive.Content>
    </DialogPortal>
